...
function UIFuncs.PopupTrigger(Title, Description, Callback)
	...
	ObjectClone.Bottom..MouseButton1Click:Connect(
		function()
			spawn(
				function()
					Callback()
...
Command.Run = function(Arguments)
	if Arguments[1] then
			...
			pcall(
				...
				elseif Aliases[caller:lower()] then
					...
					UIFuncs.SendNotification(
						"Nameless Admin",
						"O comando <b>" .. tostring(caller) .. "</b> não existe.",
						5
					)
				end
				end
		...
		if not success then
			UIFuncs.SendNotification(
				"Erro no comando!",
				"Este comando teve um erro e não pôde ser executado. Verifique seu <b>console</b> para mais informações.",
				5
			)
			warn(msg)
		end
...
Command.Add(
	...
	function()
		...
		UIFuncs.SendNotification("Nameless Admin", "Desbloqueou com sucesso todas as partes do jogo.", 5)
	end
)
...
Command.Add(
	...
	function()
		...
		UIFuncs.SendNotification("Nameless Admin", "Anti kick carregado.", 5)
	end
)
...
Command.Add(
	...
	function()
		...
		UIFuncs.SendNotification("Nameless Admin", "Anti teleport carregado.", 5)
	end
)
...
Command.Add(
	...
	function()
		...
		if Tool.Parent ~= workspace then
			...
			UIFuncs.SendNotification("Nameless Admin", "Nenhum pegador de ferramentas foi encontrado.", 5)
		end

		if FoundGrabber then
			UIFuncs.SendNotification(
				"Nameless Admin",
				"Agarrador encontrado! O nome do agarrador é: <b>" .. tostring(Grabber) .. "</b>",
				5
			)
		end
	end
...
Command.Add(
	...
	function(Username)
		if not Services.SoundService.RespectFilteringEnabled then
			...
			UIFuncs.SendNotification('Nameless Admin', 'A opção Respeitar Filtragem está ativada, então esse comando não funcionará.', 5)
		end
	end
)
...
Command.Add(
	...
	function(...)
		if tonumber(...) <= 121 then
			...
			UIFuncs.SendNotification(
				"Nameless Admin",
				"Você inseriu um número acima do máximo, alteramos seu campo de visão para 120.",
				5
			)
...
Command.Add(
	...
	function(Property, Value)
		if Property and Value then
			...
			UIFuncs.SendNotification("Nameless Admin", "Faltando um argumento.", 5)
		end
	end
)
...
Command.Add(
	...
	function()
				...
				Plr.Chatted:Connect(
					function(Message)
						if CheckIfMessageHasBannedWord(Message) then
							...
							UIFuncs.SendNotification("Nameless Admin", "Denunciado: " .. Plr.Name .. " por: " .. Reason, 5)
		...
		UIFuncs.SendNotification("Nameless Admin", "Auto denunciamento carregado!", 5)
	end
)
...
Command.Add(
	...
	function()
		...
		if fireclickdetector then
			...
			UIFuncs.SendNotification("Nameless Admin", "Acionado " .. tostring(Amount) .. " detectores de cliques.", 5)
		else
			UIFuncs.SendNotification("Nameless Admin", "Seu exploit não suporta esse comando.", 5)
		end
	end
)
...
Command.Add(
	...
	function()
		...
		UIFuncs.SendNotification("Nameless Admin", "Antifling <b>ativado</b>", 5)
		repeat
			task.wait()
...
Command.Add(
	...
	function()
		...
		UIFuncs.SendNotification(
			"Nameless Admin",
			"ControlNPCs carregado, hold the ctrl key and press on a npc for it to work",
			5
		)
	end
)
...
Command.Add(
	{"Introduction", "Int"},
	{"Introduction / Int", "Uma introdução ao Nameless Admin se você não viu ainda."},
	{"None"},
	function()
		wait(0.3)
...
Command.Add(
	...
	function()
		...
		if CommandToggles.AlreadyStartedLogging == false then
			...
			UIFuncs.SendNotification("Nameless Admin", "Registro iniciado.", 5)
			for i, Player in next, Services.Players:GetPlayers() do
				if Services.TextChatService:FindFirstChild("TextChannels") then
...
if Methods.CheckIfVulnerable() then
	...
	UIFuncs.SendNotification(
		"Nameless Admin",
		"Parece que este jogo tem um controle remoto vulnerável a ataques usando Nameless. Você recebeu comandos extras: execute <b>cmds</b> e procure por comandos que tenham '(Vuln)' ao lado para ver quais novos comandos você tem.",
		15
	)

	Command.Add(
		{"Kill", "K"},
		{"Kill / K (Vuln)", "Mata uma pessoa se o método vulnerável estiver disponível."},
		{"Username"},
		function(Username)
			local Player = PlayerFuncs.GetPlayer(Username)
	...
	Command.Add(
		{"Kick", "Kk"},
		{"Kick / Kk (Vuln)", "Expulsa uma pessoa se o método vulnerável estiver disponível."},
		{"Username"},
		function(Username)
			local Player = PlayerFuncs.GetPlayer(Username)
			for i, Player in next, Player do
	...
	Command.Add(
		{"Ban"},
		{"Ban (Vuln)", "Bane a pessoa do servidor em que você está."},
		{"Username"},
		function(Username)
			local Player = PlayerFuncs.GetPlayer(Username)
			for i, Player in next, Player do
	...
	Command.Add(
		{"Sink"},
		{"Sink (Vuln)", "Afunda um jogador no chão."},
		{"Username"},
		function(Username)
			local Player = PlayerFuncs.GetPlayer(Username)
			for i, Player in next, Player do
...
Command.Add(
	{"ClearMap", "CM"},
	{"ClearMap / CM (Vuln)", "Limpa o mapa inteiro."},
	{"None"},
	function()
...
Command.Add(
	...
	{
		"BreakGame / BG (Vuln)",
		"Destrói tudo no ReplicatedStorage, do qual a maioria dos jogos depende para funcionar."
	},
	{"None"},
	function()
...
Command.Add(
	{"Bald"},
	{"Bald (Vuln)", "Deixa seu(s) alvo(s) careca(s)."},
	{"Username"},
	function(Username)
		local Player = PlayerFuncs.GetPlayer(Username)
		for i, Player in next, Player do
...
Command.Add(
	{"Box"},
	{"Box (Vuln)", "Faz do(s) jogador(es) uma caixa."},
	{"Username"},
	function(Username)
		local Player = PlayerFuncs.GetPlayer(Username)
		for i, Player in next, Player do
...
Command.Add(
	{"Naked"},
	{"Naked (Vuln)", "Deixa o(s) jogador(es) nu(s)."},
	{"Username"},
	function(Username)
		local Player = PlayerFuncs.GetPlayer(Username)
		for i, Player in next, Player do
...
UIFuncs.SendNotification("Nameless Admin", "Nameless Admin carregou.", 5)

if gethiddenproperty then
	if gethiddenproperty(workspace, "RejectCharacterDeletions") == Enum.RejectCharacterDeletions.Disabled then
		UIFuncs.SendNotification(
			"Nameless Admin",
			"Este jogo tem o RCD desativado, o que significa que você pode usar comandos de ferramentas. Execute ;cmds para ver quais novos comandos você tem agora.",
			7
		)
	end
end
...
UIFuncs.PopupTrigger(
	"Nameless Admin",
	"Parece que você é novo em Nameless Admin, gostaria de um tutorial de uso?",
	function()
		wait(0.3)
		UIFuncs.Open(Interface.Introduction, 317, 283, 0.6)
		Interface.Introduction.Description.TextTransparency = 1
